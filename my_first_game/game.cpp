...
{
	clear_screen(0x00000); //clears the screen with a magenta color
	draw_rect(0, 0, arena_half_size_x, arena_half_size_y, 0x00000); //draws a rectangle as the background

	float player_1_ddp = 0.f; //Player 1 acceleration on the y-axis
	if (is_down(BUTTON_UP)) { player_1_ddp += 2000;}
	if (is_down(BUTTON_DOWN)) { player_1_ddp -= 2000;}

	float player_2_ddp = 0.f; //Player 2 acceleration on the y-axis
	...
	//Rendering
	draw_rect(ball_p_x, ball_p_y, ball_half_size, 1, 0x0FF00);

	draw_rect(80, player_1_p, player_half_size_x, player_half_size_y, 0xff0000); //Player 1 paddle
	draw_rect(-80, player_2_p, player_half_size_x, player_half_size_y, 0xff0000); //Player 2 paddle
}